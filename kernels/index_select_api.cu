...
// -----------------------------------------------------------------------------
// VecType helper maps (T, Vec) -> underlying vectorized type
...
__global__ void index_select_rows_vec(
    ...
    for (; j < index_count; j += j_stride) {
        ...
        // Same pattern as scalar: threads walk vector "columns" contiguously
...
static inline void compute_launch_config_rows(
    ...
    // 4–8x SMs is usually a good range for memory-bound kernels
    int max_blocks = (multi_processor_count > 0) ? (multi_processor_count * 8) : 1024;

    int blocks = static_cast<int>(index_count);
...
static void launch_index_select(
    ...
    // -----------------------------
    // Vectorized path for __half2
    // -----------------------------
    if constexpr (std::is_same<T, __half>::value) {
        // __half2 is 4 bytes (2 x 16-bit halves).
        const uintptr_t x_addr   = reinterpret_cast<uintptr_t>(x_t);
        const uintptr_t dst_addr = reinterpret_cast<uintptr_t>(dst_t);
        const bool aligned =
            ((x_addr  % alignof(__half2)) == 0) &&
            ((dst_addr % alignof(__half2)) == 0);
...
extern "C" void run_index_select(
    ...
    switch (dtype_code) {
    case 0:
        // __half: now has a __half2 fast path when aligned & cols % 2 == 0.
    ...
    case 1:
        ...
        break;
    default:
        // Unsupported dtype; Rust side should have already errored.
        break;
    }
}